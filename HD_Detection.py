...
import re
from utils import orange_color_identifier
...
class HD_Detection:
    ...
    Attributes:
        ...
        model_path (str): The path to the trained model for object detection.
    ...
    """
    def __init__(self, video_path: str, model_path:str, remove_wb:bool, orange_percentage_threshold: int, conf: float, stride:int, run_on: int, timing_inspection: int, similarity_comparision: int, label_dict) -> None:
        
        ##### checking phase
        
        self.label_dict = label_dict
        self.remove_wb = remove_wb
        self.orange_percentage_threshold = orange_percentage_threshold
        ...
        if not self.is_stride_number(stride):
            ...
            if not self.check_if_stride_is_valid(stride):
                ...
                pass
        if not self.check_if_model_exists(f"{model_path}"):
    ...
    def setup_logging(self, video_path: str):
        # Set up logging
        ...
        for idx, r in enumerate(results):
            ...
            if r.boxes.cls.size()[0]>0:
                ...
                for i in range(tensor_data.size(0)):
                    ...
                    if predicted_label == "flag":
                        # crop the predicted object
                        crop_img = frame_list[idx][int(y):int(h), int(x):int(w)]
                        # check with the color identifier
                        if orange_color_identifier(crop_img, self.orange_percentage_threshold) == False:
                            continue
